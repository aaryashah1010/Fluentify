      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-slate-900 dark:text-slate-50 text-4xl md:text-5xl mb-4">
            Everything You Need to <span className="text-teal-600 dark:text-teal-400">Master</span> a Language
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {features.map((feature, index) => {
            ...
            const iconColor = feature.color === 'teal' ? 'text-teal-600 dark:text-teal-300' : 'text-orange-600 dark:text-orange-300';
            const bgColor = feature.color === 'teal' ? 'bg-teal-100 dark:bg-teal-500/20' : 'bg-orange-100 dark:bg-orange-500/20';

            return (
              <div
                key={index}
                className="bg-white/80 dark:bg-slate-900/80 rounded-2xl p-8 border border-slate-200 dark:border-white/10 hover:shadow-2xl transition-all duration-300 hover:scale-105"
              >
                <div className={`w-14 h-14 ${bgColor} rounded-xl flex items-center justify-center mb-6`}>
                  <Icon className={`w-7 h-7 ${iconColor}`} />
                </div>
                <h3 className="text-slate-900 dark:text-slate-50 text-xl mb-3">{feature.title}</h3>
