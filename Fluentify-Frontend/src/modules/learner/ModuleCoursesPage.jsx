    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-50 dark:from-teal-900 via-orange-50 dark:via-orange-900 to-slate-100 dark:to-slate-950 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="text-center z-10">
          ...
          <p className="text-slate-700 dark:text-slate-200 font-medium animate-pulse">Loading {language} courses...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        ...
        {error && (
          <div className="bg-red-100 dark:bg-red-900/50 border border-red-300 dark:border-red-500/70 rounded-2xl p-6 mb-8 flex items-start gap-4">
             ...
             <div>
                <h3 className="text-red-700 dark:text-red-100 font-semibold">Unable to load courses</h3>
                <p className="text-red-700 dark:text-red-200 mt-1">{error.message || 'Failed to load courses'}</p>
             </div>
          </div>
        )}
        ...
        {courses.length === 0 ? (
          <div className="bg-white dark:bg-slate-900/85 rounded-3xl p-12 text-center shadow-xl border border-slate-200 dark:border-white/10">
            ...
            <h3 className="text-xl font-semibold text-slate-900 dark:text-slate-50 mb-2">No courses found</h3>
            <p className="text-slate-600 dark:text-slate-300">Check back soon for new instructor-led content.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {courses.map((course) => (
              <button
                ...
                className="group bg-white dark:bg-slate-900/85 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 border border-slate-200 dark:border-white/10 hover:border-teal-300 dark:hover:border-teal-400/60 flex flex-col overflow-hidden text-left h-full transform hover:-translate-y-1"
              >
                ...
                <div className="p-6 flex-1 flex flex-col">
                  <h3 className="text-xl font-bold text-slate-900 dark:text-slate-50 mb-2 line-clamp-2 group-hover:text-teal-600 dark:group-hover:text-teal-300 transition-colors">
                    {course.title}
                  </h3>
                  <p className="text-slate-600 dark:text-slate-300 text-sm mb-6 line-clamp-2 flex-1">
                    {course.description || 'No description provided.'}
                  </p>

                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-200 dark:border-white/10 text-xs text-slate-600 dark:text-slate-300">
                    <div className="flex items-center gap-1.5">
                      <GraduationCap className="w-4 h-4 text-orange-600 dark:text-orange-300" />
                      <span>{course.total_units || 0} Units</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Signal className="w-4 h-4 text-teal-600 dark:text-teal-300" />
