...
const CourseCard = ({ course, onClick }) => {
  ...
  return (
    ...
    <div className="bg-white/90 dark:bg-slate-900/85 rounded-2xl shadow-xl border border-slate-200 dark:border-white/10 p-6 hover:shadow-2xl hover:border-teal-400/70 transition-all overflow-hidden">
      {/* Delete Confirmation Modal */}
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-slate-950 rounded-2xl p-6 max-w-sm mx-4 border border-red-400 dark:border-red-500/40 shadow-2xl">
            <div className="text-center text-slate-800 dark:text-slate-100">
              <AlertTriangle className="w-8 h-8 text-red-500 mx-auto mb-2" />
              <h4 className="font-semibold text-slate-900 dark:text-slate-50 mb-2">Delete Course?</h4>
              ...
              <div className="flex gap-2 justify-center">
                <button
                  ...
                  className="px-4 py-2 text-sm bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700"
                >
                  Cancel
                </button>
                <button
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1 min-w-0">
          <h4 className="font-semibold text-lg text-slate-900 dark:text-slate-50 mb-1">{course.title}</h4>
          <p className="text-sm text-slate-500 dark:text-slate-300 mb-2">{course.language}</p>
        ...
        <div className="flex flex-col gap-2 items-end flex-shrink-0">
          <div className="w-10 h-10 bg-slate-100 dark:bg-slate-800/80 rounded-xl flex items-center justify-center border border-slate-200 dark:border-white/10 overflow-hidden">
          ...
          <button
            onClick={handleDelete}
            disabled={deleting}
            className="w-10 h-10 rounded-xl border border-red-300 dark:border-red-500/50 bg-white dark:bg-slate-900/80 flex items-center justify-center text-slate-500 dark:text-slate-300 hover:bg-red-100 dark:hover:bg-red-500/20 hover:text-red-600 dark:hover:text-red-200 transition-colors disabled:opacity-50 overflow-hidden"
            title="Delete Course"
          >
      ...
      <div className="space-y-2 mb-4">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="font-semibold text-teal-600 dark:text-teal-300">{progressPercentage}%</span>
        </div>
