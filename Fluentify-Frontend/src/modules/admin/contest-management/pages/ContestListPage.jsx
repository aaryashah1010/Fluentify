...
const ContestListPage = () => {
  ...
  const getStatusBadge = (status) => {
    const badges = {
      DRAFT: 'bg-white dark:bg-slate-900/80 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-white/10',
      PUBLISHED: 'bg-sky-50 dark:bg-sky-500/15 text-sky-600 dark:text-sky-300 border border-sky-300 dark:border-sky-400/60',
      ACTIVE: 'bg-emerald-50 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-400/60',
      ENDED: 'bg-rose-50 dark:bg-rose-500/15 text-rose-600 dark:text-rose-300 border border-rose-300 dark:border-rose-400/60',
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-50 dark:from-teal-900 via-orange-50 dark:via-orange-900 to-slate-100 dark:to-slate-950 text-slate-900 dark:text-slate-50">
        ...
        <div className="w-full max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-slate-50">Contest Management</h1>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="relative overflow-hidden rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900/85 p-6 animate-pulse"
              >
          ...
          <div className="text-center py-16 rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950/90 shadow-2xl">
            <Trophy className="w-16 h-16 text-amber-600 dark:text-amber-300 mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-slate-900 dark:text-slate-50 mb-2">No Contests Yet</h3>
            <p className="text-slate-600 dark:text-slate-300 mb-6">Create your first contest to get started.</p>
            <Button
              onClick={() => navigate('/admin/contests/new')}
              icon={<Plus className="w-4 h-4" />}
              className="bg-gradient-to-r from-teal-500 to-orange-500 text-slate-950 rounded-xl hover:from-teal-600 hover:to-orange-600"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {contests.map((contest) => (
              <div
                key={contest.id}
                className="relative overflow-hidden rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900/85 p-6 shadow-xl hover:shadow-2xl hover:scale-[1.01] transition-all"
              >
                {/* Glow blob */}
                ...
                {/* Contest Details */}
                <div className="space-y-2 mb-4 text-xs text-slate-600 dark:text-slate-300 relative z-10">
                  <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gradient-to-r from-teal-500 to-orange-500 border border-slate-300 dark:border-white/15 text-white dark:text-slate-300">
                    <Calendar className="w-4 h-4 text-white dark:text-amber-300" />
                ...
                {/* Description */}
                {contest.description && (
                  <p className="text-xs text-slate-600 dark:text-slate-300 mb-4 line-clamp-2 relative z-10">
                    {contest.description}
                  </p>
                )}

                {/* Actions */}
                <div className="flex items-center gap-2 pt-4 border-t border-slate-200 dark:border-white/10 relative z-10">
                  <button
                    onClick={() => navigate(`/admin/contests/${contest.id}`)}
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-xs font-medium text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-900/80 hover:bg-slate-100 dark:hover:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-white/10 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-3 py-2 text-xs font-medium text-rose-700 dark:text-rose-200 bg-rose-100 dark:bg-rose-950/70 hover:bg-rose-100 dark:hover:bg-rose-900/80 rounded-xl border border-rose-300 dark:border-rose-500/50 transition-colors"
                  >
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-slate-950/95 border border-slate-200 dark:border-white/10 rounded-3xl p-6 max-w-md w-full mx-4 shadow-2xl">
            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-50 mb-2">Delete Contest?</h3>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex-1 px-4 py-2 text-xs font-medium text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-900/80 hover:bg-slate-100 dark:hover:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-white/10 transition-colors"
              >
                Cancel
              </button>
              <button
