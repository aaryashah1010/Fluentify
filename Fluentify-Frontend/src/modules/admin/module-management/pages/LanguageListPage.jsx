...
const LanguageListPage = () => {
  ...
  if (error) {
    return (
      <div className="rounded-3xl border border-rose-300 dark:border-rose-500/60 bg-rose-100 dark:bg-rose-950/70 p-6 text-sm text-rose-700 dark:text-rose-100">
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-slate-50">Language Modules</h2>
          <p className="text-sm text-slate-600 dark:text-slate-300 mt-1">Select a language to manage its courses.</p>
        </div>
        {languages.length > 0 && (
          <button
      ...
      {languages.length === 0 ? (
        <div className="bg-white dark:bg-slate-900/85 border border-slate-200 dark:border-white/10 rounded-3xl p-8 text-center">
          <Globe className="w-12 h-12 text-slate-500 dark:text-slate-400 mx-auto mb-3" />
          <p className="text-slate-600 dark:text-slate-300 mb-4">No languages found. Create your first course to get started.</p>
          <button
            onClick={() => navigate('/admin/modules/course/new')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {languages.map((lang) => (
            <button
              ...
              className="relative overflow-hidden rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900/85 p-6 text-left group shadow-xl hover:shadow-2xl hover:scale-[1.01] transition-all"
            >
              ...
              <div className="relative flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-white/10 rounded-2xl flex items-center justify-center">
                    <Globe className="w-6 h-6 text-teal-600 dark:text-teal-300" />
                  </div>
                  <div>
                    <h3 className="font-semibold text-slate-900 dark:text-slate-50 text-lg">{lang.language}</h3>
                    <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">
                      {lang.course_count} {lang.course_count === '1' ? 'course' : 'courses'}
                    </p>
                  </div>
                </div>
                <ChevronRight className="w-5 h-5 text-slate-500 dark:text-slate-400 group-hover:text-teal-600 dark:group-hover:text-teal-300 transition-colors" />
              </div>
            </button>
          ))}
        </div>
