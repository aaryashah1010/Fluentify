...
const ContestCard = ({ contest, isAdmin = false }) => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'DRAFT':
        return 'bg-slate-200 text-slate-700 dark:bg-slate-800/70 dark:text-slate-100';
      case 'PUBLISHED':
  ...
  return (
    <div className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-white/10 rounded-2xl p-6 shadow-lg hover:shadow-xl hover:border-cyan-400/60 transform hover:-translate-y-0.5 transition-all">
      {/* Header */}
      <div className="flex justify-between items-start mb-4">
        <h3 className="text-xl font-semibold text-slate-900 dark:text-white">{contest.title}</h3>
      ...
      {/* Description */}
      {contest.description && (
        <p className="text-slate-500 dark:text-slate-300 mb-4 line-clamp-2">{contest.description}</p>
      )}

      {/* Contest Info */}
      <div className="space-y-2 mb-4">
        <div className="flex items-center text-sm text-slate-500 dark:text-slate-300">
