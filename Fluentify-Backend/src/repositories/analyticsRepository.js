...
class AnalyticsRepository {
   ...
   * FIX: Now pulls from actual lesson_progress table for real-time accuracy
   */
    ...
    try {
      const query = `
        SELECT 
          ...
          COUNT(DISTINCT lp.id)::int as count
        ...
        SELECT 
          ...
          COUNT(DISTINCT lp.id)::int as count
        FROM lesson_progress lp
        INNER JOIN language_modules lm ON lp.course_id = lm.id
        WHERE lp.is_completed = true
          AND lm.language IS NOT NULL
